...
def fetch_hackerone_vdps(limit=25):
    ...
    try:
        ...
        print(f"[HackerOne DEBUG] status={resp.status_code}, "
              f"response length={len(resp.text)} chars")
        print(f"[HackerOne DEBUG] first 300 chars: {resp.text[:300]}")
    ...
    if not results:
        print("[HackerOne] Request succeeded but no programs were parsed — "
              "the API response shape likely changed since this script was written.")
        results.append({
            "name": "HackerOne VDP Directory (manual check — API fetch returned nothing)",
...
def fetch_bugcrowd_vdps(limit=25):
    ...
    try:
        ...
        print(f"[Bugcrowd DEBUG] status={resp.status_code}, "
              f"response length={len(resp.text)} chars")
        print(f"[Bugcrowd DEBUG] first 300 chars: {resp.text[:300]}")
        resp.raise_for_status()
        data = resp.json()
    ...
    if not results:
        print("[Bugcrowd] Request succeeded but no programs were parsed — "
              "the API response shape likely changed since this script was written.")
        results.append({
            "name": "Bugcrowd VDP Directory (manual check — API fetch returned nothing)",
